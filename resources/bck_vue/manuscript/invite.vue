            ...
            <div class="mb-4">
                ...
                <div class="overflow-y-auto rounded">
                    <table class="table text-left text-sm">
                        ...
                        <tbody>
                            <template
                                v-if="manuscript.revision?.reviewers?.filter(x => x.section.value === 'author') && manuscript.revision?.reviewers?.filter(x => x.section.value === 'author').length > 0">
                                <tr v-for="(revision_reviewer, i) in manuscript.revision?.reviewers?.filter(x => x.section.value === 'author')"
                                    :key="`row-${i}`">
                                    <td>{{ i + 1 }}</td>
                                    <td>
                                        <div class="relative">
