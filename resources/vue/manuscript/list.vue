...
type Props = {
    ...
    formatterlist?:Author[];
    proofreaderlist?:Author[];
};
...
const updateFormatterForm = useForm({
    email:undefined as unknown as string | undefined
});
...
function action($event: Event, row: Manuscript) {
    ...
    switch (action) {
        ...
        case 'proofreader':
            updateManuscript.value = row;
            axios.post(`/manuscripts/${row.id}/get-proofreader`)
            .then(response => {
                    checkProofreader = response.data;
...
<template>
    <Layout :title="title">
        ...
        <Modal v-if="formatterDialogModalAction && formatterlist" title="Formatter" size="xl" @close="formatterDialogModalAction = false">
            <div class="p-3">
                <h4 class="text-center text-lg py-2 underline font-bold">Formatter List</h4>
                <table class="table text-left text-sm">
                        ...
                        <tbody>
                            <template v-if="formatterlist.length > 0">
                                <label v-for="(row, i) in formatterlist" :key="`row-${i}`" :for="`formatter-list-${row.id}`" role="table-row" class="table-row">
                                    <td class="align-middle">
                                    <input v-model="updateFormatterForm.email" type="radio" :id="`formatter-list-${row.id}`" class="w-4 h-4" :value="row.email" />
                                    </td>
                                    <td>{{ row.name ?? '-' }}</td>
                                    <td>{{ row.email ?? '-' }}</td>
                                ...
                                <div v-if="updateFormatterForm.errors?.email" class="text-sm text-red-700">{{ updateFormatterForm.errors.email }}</div>
                            </template>
                            <template v-else>
                                <tr>
                ...
                <div class="py-2 flex justify-center space-x-1">
                    ...
                    <template v-else>
                        <button type="button" class="text-sm py-2 px-4 bg-primary-500 text-white" @click.prevent="updateFormatterForm.post(`/manuscripts/${updateManuscript?.id}/update-formatter`, { onSuccess: () => formatterDialogModalAction = false })"> Save
                        </button>
        ...
        <Modal v-if="proofreaderDialogModalAction && proofreaderlist" title="Proof Read" size="xl" @close="proofreaderDialogModalAction = false">
            <div class="p-3">
                <h4 class="text-center text-lg py-2 underline font-bold">Proof Read List</h4>
                <div v-if="checkProofreader === 1">
                    <table class="table text-left text-sm">
                        <thead class="uppercase">
                            <tr>
                                <th></th>
                        ...
                        <tbody>
                            <template v-if="proofreaderlist.length > 0">
                                <label v-for="(row, i) in proofreaderlist" :key="`row-${i}`" :for="`proofreader-list-${row.id}`" role="table-row" class="table-row">
                                    <td class="align-middle">
                                    <input v-model="updateProofreaderForm.email" type="radio" :id="`proofreader-list-${row.id}`" class="w-4 h-4" :value="row.email" />
                                    </td>
                                    <td>{{ row.name ?? '-' }}</td>
                                    <td>{{ row.email ?? '-' }}</td>
                                    <td>{{ row.highest_qualification ?? '-' }}</td>
                                ...
                                <div v-if="updateProofreaderForm.errors?.email" class="text-sm text-red-700">{{ updateProofreaderForm.errors.email }}</div>
                            </template>
                            <template v-else>
                                <tr>
                                    <td colspan="7" class="py-2 text-center">
                                        <p class="text-gray-800 text-base">No proofreader found.</p>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
                    ...
                    <div class="py-2 flex justify-center space-x-1">
                        ...
                        <template v-else>
                            <button type="button" class="text-sm py-2 px-4 bg-primary-500 text-white" @click.prevent="updateProofreaderForm.post(`/manuscripts/${updateManuscript?.id}/update-proofreader`, { onSuccess: () => proofreaderDialogModalAction = false })"> Save
                            </button>
                ...
                <div v-else>
                    <p class="text-center text-gray-800 text-base p-5">Already Send.</p>
                </div>
            </div>
        </Modal>
