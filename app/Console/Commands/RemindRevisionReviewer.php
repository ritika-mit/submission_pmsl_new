...
class RemindRevisionReviewer extends Command
{
    ...
    public function handle()
    {
        RevisionReviewer::query()
            ...
            ->whereRaw('IFNULL(`remind_count`, 0) < ?', 2)
            ->whereRaw('DATE_ADD(`accepted_at`, INTERVAL IFNULL(`remind_count`, 1) * ? DAY) < CURDATE()', 10)
            ...
            ->each(function ($revision_reviewer) {
                ...
                $this->info('Review reminder E-mail sent to '. $reviewerId);
                sleep(1);
            });
            Log::info("Revision.manuscript Reminder E-mail sent:");    
            $this->info('Revision.manuscript Reminder E-mail sent at ' . now()->setTimezone('Asia/Kolkata')->toDateTimeString() . ' IST');
